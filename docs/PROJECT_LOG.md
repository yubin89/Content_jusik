...
- **조건**: **최근 3거래일** 동안 **매일** `(외국인합계 + 기관합계) > 0` (외인·기관 **합산** 순매수)
  - 한쪽이 조금 팔아도 합쳐서 순매수면 인정. (2026-06-14: '둘 다 각각 순매수' → '합산 순매수'로 완화)
...
- **거래량 급증**: 최근일 거래량 ≥ 최근 20거래일 평균 ×`VOL_SURGE_MULT`(기본 2배) → 관심·자금 유입 신호
- **52주 신고가권**: 최근 종가 ≥ 252거래일 최고 종가 ×`HIGH_NEAR_RATIO`(기본 0.97) → 추세 돌파 신호
- **🔥 3중 신호**: 수급 + 거래량 급증 + 신고가권 **동시 충족** → 노션 페이지 맨 위 + 텔레그램에 별도 강조
- 노션 C 페이지 구성: ① 🔥3중 신호 종목, ② 전체 수급 종목(각 종목에 `거래량 N배 / 신고가` 태그)
...
| 수급: 판정 방식 | (외인+기관) **합산** 순매수 | `scripts/pykrx_scan.py` `analyze_ticker` |
| 거래량 급증 배수 | `VOL_SURGE_MULT = 2.0` | `scripts/pykrx_scan.py` 상단 |
| 52주 신고가권 비율 | `HIGH_NEAR_RATIO = 0.97` | `scripts/pykrx_scan.py` 상단 |
